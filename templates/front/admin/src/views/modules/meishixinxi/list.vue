<template>
	<div class="main-content" :style='{"padding":"30px","flexWrap":"wrap","display":"flex"}'>
			...
			<el-form class="center-form-pv" :style='{"width":"80%","margin":"0 0 20px","alignItems":"flex-start","flexDirection":"column","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #654B3C","width":"auto","overflow":"hidden","borderRadius":"20px","display":"inline-block"}' >
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">菜谱名</label>
						<el-input v-model="searchForm.name" placeholder="菜谱名" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"20px 0","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('meishixinxi','爬取数据')" type="success" @click="spider()">
						<span class="icon iconfont icon-shuju10" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						爬取数据
					</el-button>

					<el-button class="btn18" v-if="isAuth('meishixinxi','菜品浏览')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						菜品浏览
					</el-button>
					<el-button class="btn18" v-if="isAuth('meishixinxi','收藏量')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						收藏量
					</el-button>
					<el-button class="btn18" v-if="isAuth('meishixinxi','发布量')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						发布量
					</el-button>
				</el-row>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="viewChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="collectChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="authorChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var viewChart1 = echarts.init(document.getElementById("viewChart1"),'macarons');
        this.$http({
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var collectChart2 = echarts.init(document.getElementById("collectChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                collectChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    collectChart2.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var authorChart3 = echarts.init(document.getElementById("authorChart3"),'macarons');
        this.$http({
