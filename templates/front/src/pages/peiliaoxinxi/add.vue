    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="配料名称" prop="peiliaomingcheng">
            <el-input v-model="ruleForm.peiliaomingcheng" 
                placeholder="配料名称" clearable :disabled=" false  ||ro.peiliaomingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="配料分类" prop="peiliaofenlei">
            <el-input v-model="ruleForm.peiliaofenlei" 
                placeholder="配料分类" clearable :disabled=" false  ||ro.peiliaofenlei"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="材料详情" prop="cailiaoxiangqing">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`peiliaoxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('peiliaoxinxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`peiliaoxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
